...
# File paths
file1 = r"C:\Users\mthompson\OneDrive - Integrity Senior Services\Desktop\Missed Appt. Tracker Output\The stuff\Merged - All Parts Combined.xlsx"
file2 = r"C:\Users\mthompson\OneDrive - Integrity Senior Services\Desktop\Missed Appt. Tracker Output\Log for remaining Run\Final Missing\remainder.xlsx"
output_file = r"C:\Users\mthompson\OneDrive - Integrity Senior Services\Desktop\Missed Appt. Tracker Output\The stuff\Final Merged - Complete.xlsx"
...
print("=" * 80)
print("MERGING TWO EXCEL FILES WITH PROPER FORMAT")
print("=" * 80)
...
try:
    ...
    df1['_source_file'] = 'Merged - All Parts Combined'
    print(f"   ✓ File 1: {len(df1)} client rows")
...
try:
    ...
    df2['_source_file'] = 'Remainder'
    print(f"   ✓ File 2: {len(df2)} client rows")
except Exception as e:
...
print(f"   ✓ Found {len(duplicate_counselors_list)} counselors appearing in both files")
...
for idx, row_data in merged_df.iterrows():
    ...
    for col_idx, header in enumerate(headers, 1):
        ...
        # Ensure client rows have white background (no fill) and black text
        cell.fill = PatternFill()  # No fill (white/transparent background)
        cell.font = Font(name='Calibri', size=11, color='000000')  # Black text
        cell.alignment = Alignment(horizontal='left', vertical='center')
...
print(f"   • Total before merge: {len(df1) + len(df2)}")
print(f"   • Final merged rows: {len(merged_df)}")
print(f"   • Rows removed (duplicates): {len(df1) + len(df2) - len(merged_df)}")
