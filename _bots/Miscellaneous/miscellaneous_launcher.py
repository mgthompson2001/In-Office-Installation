# Miscellaneous Launcher
# Sub-launcher for Miscellaneous utility bots
...
class MiscellaneousLauncher(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Miscellaneous - Bot Launcher - Version 3.1.0, Last Updated 12/04/2025")
        self.geometry("750x800")
        self._setup_bot_configs()
        self._build_ui()

    def _setup_bot_configs(self):
        """Configure all bots in the Miscellaneous department"""
        from pathlib import Path
        
        # Get base installation directory (go up from _bots/Miscellaneous to In-Office Installation)
        installation_dir = Path(__file__).parent.parent.parent
        
        self.bots = {
            "Document Translator": {
                "path": str(installation_dir / "_bots" / "Miscellaneous" / "Document Translator" / "document_translator_bot.py"),
                "description": "Translate documents (PDF, Word, text files) to any language using OCR and translation services. Supports scanned PDFs via OCR, Word documents, and plain text files."
            }
        }
    ...
    def _build_ui(self):
        ...
        header.pack(fill="x")
        tk.Label(header, text="Miscellaneous", bg="#660000", fg="white",
